   ...
   try {
    ...
    try {
      ...
      if (rec) {
        ...
        /* every entry must be a URL string: a list like [123] matches no
           filename, so every asset would look up to date, stale copies would be
           skipped and the sweep would run on a cache that is not complete */
        if (o && typeof o.build === "string" && Array.isArray(o.pending) &&
            o.pending.every((u) => typeof u === "string")) meta = o;
      }
    } catch (err) {}
